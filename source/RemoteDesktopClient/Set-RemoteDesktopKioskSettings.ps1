...
This integer parameter specifies the maximum random delay in hours that can be added to the maintenance activation time to prevent multiple systems from running maintenance simultaneously. Valid values are 0-6 hours. The value is converted to ISO 8601 duration format (PT#H) internally. Default is 2 hours.
...
Else {
    ...
    If ($DeviceRemovalAction -and -not ($SmartCard.IsPresent) -and ($null -eq $DeviceVendorID -or $DeviceVendorID -eq '')) {
...
If ($InstallRemoteDesktopClient) {
    ...
    & "$DirApps\VisualC++Redistributables\Deploy-VisualC++Redistributables.ps1"
    ...
    & "$DirApps\RemoteDesktopClient\Deploy-RemoteDesktopClient.ps1"
}
...
Else {
    # These settings are already included in the SharedPC provisioning package, so only add them when not using SharedPC mode.
    $ProvisioningPackages += [PSCustomObject]@{

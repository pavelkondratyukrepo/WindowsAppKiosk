...
If ($DeploymentType -ne 'UnInstall') {
    ...
    # Check if MSI file exists in script directory (offline installation)
    $LocalMSI = Get-ChildItem -Path $PSScriptRoot -Filter *.msi -ErrorAction SilentlyContinue
    
    If ($LocalMSI) {
        Write-Output "Remote Desktop MSI package found in $PSScriptRoot"
        $pathMSI = $LocalMSI.FullName
    }
    Else {
        ...
        Write-Output "Remote Desktop MSI package downloaded to: $pathMSI"
    }
    ...
    If (-not($Installed) -or $AvailableVersion -gt $InstalledVersion) {
        ...
        If ($($Installer.ExitCode) -eq 0) {
            ...
            # Only remove temp file if it was downloaded (not local)
